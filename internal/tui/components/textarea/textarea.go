...
import (
	...
	"tui/components/textarea/memoization"
	"tui/components/textarea/runeutil"
)
...
// SetGhost configures a transient hint to render at the current cursor
// position without modifying the underlying buffer.
func (m *Model) SetGhost(text string, style lipgloss.Style) {
	m.ghostActive = text != ""
	if !m.ghostActive {
		m.ghostText = ""
		return
	...
	if style.GetForeground() == nil {
		m.ghostStyle = m.activeStyle().computedPlaceholder()
	} else {
		m.ghostStyle = style
	}
	m.ghostRow = clamp(m.row, 0, len(m.value)-1)
	if m.ghostRow < len(m.value) && m.ghostRow >= 0 {
		line := m.value[m.ghostRow]
		m.ghostCol = clamp(m.col, 0, len(line))
	} else {
		m.ghostCol = m.col
	}
}

// ClearGhost removes any active ghost hint from the textarea.
func (m *Model) ClearGhost() {
	m.ghostActive = false
	m.ghostText = ""
}
...
func (m Model) lineInfoAt(row, col int) LineInfo {
	...
	for i, line := range grid {
		...
		if counter+len(line) >= col {
			runeOffset := col - counter
			charOffset := uniseg.StringWidth(string(line[:max(0, runeOffset)]))
			return LineInfo{
				CharOffset:   charOffset,
				ColumnOffset: runeOffset,
				Height:       len(grid),
				RowOffset:    i,
				StartColumn:  counter,
				Width:        len(line),
	...
	return LineInfo{
		CharOffset:   uniseg.StringWidth(string(last)),
		ColumnOffset: len(last),
		Height:       len(grid),
		RowOffset:    len(grid) - 1,
		StartColumn:  counter - len(last),
		Width:        len(last),
		CharWidth:    uniseg.StringWidth(string(last)),
	}
}
...
func (m *Model) view() string {
	...
	ghostActive := m.ghostActive && m.ghostText != ""
	ghostStyle := m.ghostStyle
	ghostInfo := LineInfo{}
	if ghostActive {
		ghostInfo = m.lineInfoAt(m.ghostRow, m.ghostCol)
	}
	ghostRendered := false
	...
	for l, line := range m.value {
		...
		for wl, wrappedLine := range wrappedLines {
			...
			if ghostActive && !ghostRendered && l == m.ghostRow && ghostInfo.RowOffset == wl {
				ghostWidth := uniseg.StringWidth(m.ghostText)
				s.WriteString(ghostStyle.Render(m.ghostText))
				ghostRendered = true
				if padding > ghostWidth {
					padding -= ghostWidth
				} else {
					padding = 0
				}
