...
type Model struct {
	...
	// Scroll event tracking for detecting rapid scrolling
	lastScrollTime      time.Time
	scrollEventCount    int
	scrollEventInterval time.Duration
}
...
// IsRapidScrolling returns true if scroll events are coming in rapidly.
// This can happen when the user does a quick swipe on a touchpad.
// The threshold is 50ms - events closer together than this are considered rapid.
func (m Model) IsRapidScrolling() bool {
	return m.scrollEventInterval > 0 && m.scrollEventInterval < 50*time.Millisecond
}

// ScrollEventCount returns the number of consecutive scroll events.
// This can be used to detect when multiple scroll events are queued up.
func (m Model) ScrollEventCount() int {
	return m.scrollEventCount
}

// ResetScrollTracking resets the scroll event tracking.
func (m *Model) ResetScrollTracking() {
	m.scrollEventCount = 0
	m.lastScrollTime = time.Time{}
	m.scrollEventInterval = 0
}

// adaptiveScrollDelta returns an adjusted scroll delta based on scroll speed.
// This prevents runaway scrolling from rapid touchpad swipes while maintaining
// precise control for deliberate scrolling.
//
// Returns:
//   - Full MouseWheelDelta for normal scrolling (>50ms between events)
//   - 2 lines for fast scrolling (20-50ms between events)
//   - 1 line for very fast scrolling (<20ms between events)
func (m Model) adaptiveScrollDelta() int {
	if m.scrollEventInterval == 0 {
		// First scroll event, use normal delta
		return m.MouseWheelDelta
	}

	// Very fast scrolling (touchpad swipe) - use minimal delta
	if m.scrollEventInterval < 20*time.Millisecond {
		return 1
	}

	// Fast scrolling - use reduced delta
	if m.scrollEventInterval < 50*time.Millisecond {
		return min(2, m.MouseWheelDelta)
	}

	// Normal scrolling - use full delta
	return m.MouseWheelDelta
}
...
func (m Model) updateAsModel(msg tea.Msg) Model {
	...
	case tea.MouseWheelMsg:
		...
		// Track scroll event timing for rapid scroll detection
		now := time.Now()
		if !m.lastScrollTime.IsZero() {
			m.scrollEventInterval = now.Sub(m.lastScrollTime)
			m.scrollEventCount++
		} else {
			m.scrollEventCount = 1
		}
		m.lastScrollTime = now

		// Get adaptive delta based on scroll speed
		delta := m.adaptiveScrollDelta()
